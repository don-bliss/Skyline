...
async function sendEmailCode(email,code){if(!RESEND_API_KEY||!EMAIL_FROM)return {sent:false,reason:'email provider not configured'};const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${RESEND_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({from:EMAIL_FROM,to:[email],subject:'Your SkyLine verification code',html:`<div style="font-family:Arial"><h2>Verify your SkyLine email</h2><p>Your code is <b style="font-size:24px;letter-spacing:4px">${code}</b></p><p>This code expires in 10 minutes.</p></div>`})});if(!r.ok)throw Error('Email delivery failed');return {sent:true};}
...
app.get('/api/v1/wallet',requireAuth,async(req,res)=>{try{const tx=pool?(await pool.query('SELECT id,type,amount,status,description,created_at FROM skyline_transactions WHERE user_id=$1 ORDER BY created_at DESC LIMIT 20',[req.user.id])).rows.map(r=>({id:r.id,type:r.type,amount:Number(r.amount),status:r.status,description:r.description,createdAt:r.created_at})):[];res.json({wallet:{accountNumber:req.user.accountNumber,accountName:req.user.name,balance:Number(req.user.balance||0),currency:'NGN',phone:req.user.phone,email:req.user.email,phoneVerified:!!req.user.phoneVerified,emailVerified:!!req.user.emailVerified},transactions:tx});}catch{res.status(500).json({message:'Unable to load wallet.'});}});
