...
const publicProfileUser=u=>({id:u.id,name:u.name,username:u.username,avatar:u.avatar||'',bio:u.bio||'',location:u.location||'',website:u.website||'',createdAt:u.createdAt});
...
app.get('/api/v1/public/profile/:username',async(req,res)=>{try{const u=await findUserByUsername(req.params.username);if(!u)return res.status(404).json({message:'Profile not found.'});res.json({user:publicProfileUser(u)});}catch(e){res.status(500).json({message:'Unable to load profile.'});}});
