...
(() => {
  ...
  if(signup){
    ...
    let generated = ''; const phoneInput=document.getElementById('phone'); const countryInput=document.getElementById('phoneCountry'); const derive=()=>{let d=String(phoneInput?.value||'').replace(/\D/g,'').replace(/^0+/,''); return d.length>=6?d:'';}; const updateIdentity=()=>{generated=derive(); if(account) account.textContent=generated||'Enter your phone number';}; phoneInput?.addEventListener('input',updateIdentity); countryInput?.addEventListener('change',updateIdentity); updateIdentity();
    ...
    signup.addEventListener('submit',async e=>{e.preventDefault();const name=document.getElementById('fullName').value.trim(),email=document.getElementById('email').value.trim(),phone=document.getElementById('phone').value.trim(),country=document.getElementById('phoneCountry')?.value||'NG',password=pass.value,terms=document.getElementById('terms').checked;if(!name||!email||!phone||password.length<8||!terms||password!==confirm.value){alert('Please complete the form and make sure your password meets all requirements.');return;}const btn=document.getElementById('createAccountBtn');btn.disabled=true;btn.textContent='Creating account...';let avatar='';if(photo?.files?.[0]){avatar=await new Promise(res=>{const r=new FileReader();r.onload=e=>res(e.target.result);r.readAsDataURL(photo.files[0]);});}try{if(window.skylineAuthReady) await window.skylineAuthReady; if(typeof window.skylineSignUp!=='function') throw new Error('SkyLine authentication is unavailable. Please refresh the page and try again.'); const result=await window.skylineSignUp({name,email,phone,country,password,avatar});document.getElementById('createdAccountNumber').textContent=result.user.accountNumber||generated;document.getElementById('verificationEmail').textContent=email;const demo=document.getElementById('demoVerificationCode');if(demo){if(window.SkyLineAPI?.isConfigured?.()){demo.textContent='Secure account created on the SkyLine server.';}else{const code=String(Math.floor(100000+Math.random()*900000));const verification={email:email.trim().toLowerCase(),code,createdAt:Date.now(),expiresAt:Date.now()+10*60*1000,verified:false};sessionStorage.setItem('skylinePendingVerification',JSON.stringify(verification));demo.textContent='Local test mode code: '+code;}demo.hidden=false;}document.getElementById('successOverlay')?.classList.add('show');setTimeout(()=>location.href='profile.html?verify=1',1000);}catch(err){console.error('SkyLine signup error:',err);alert(err?.message||'Unable to create your account. Please refresh the page and try again.');btn.disabled=false;btn.textContent='Create Account';}});
  }
  ...
  if(login){
    ...
    login.addEventListener('submit',async e=>{e.preventDefault();const em=email.value.trim(),pw=password.value;if(!em||!pw)return;const btn=document.getElementById('loginBtn'),txt=document.getElementById('loginBtnText');btn.disabled=true;txt.textContent='Signing in...';try{const u=await window.skylineSignIn(em,pw);if(remember.checked)localStorage.setItem('skylineRememberedEmail',em);else localStorage.removeItem('skylineRememberedEmail');localStorage.setItem('skylineUser',JSON.stringify(u));document.getElementById('loginSuccess')?.classList.add('show');setTimeout(()=>location.replace((u.emailVerified&&u.phoneVerified)?'timeline.html?v=20260907-v23&login=1':'profile.html?verify=1'),900);}catch(err){alert(err.message||'Unable to sign in.');btn.disabled=false;txt.textContent='Login';}});
